...
EQU = {
    ...
    "BLANKS":  0xCF38,
    "TMP":     0xCF39,
    "TXW":     0xCFFD,   # last timer-A high byte seen by txwait
    "TMP2":    0xCF3A,
...
;
; Reading $dd0d CLEARS it, and on a real 6526 a read colliding with the
; timer setting its underflow flag loses that flag -- the bit then runs to
; double length. VICE does not reproduce it, so every emulator test
; passed.
;
; This variant arms a ONE-SHOT per bit and polls the START bit in the
; control register, which the CIA clears by itself when a one-shot times
; out. Reading the control register has no side effects whatsoever, and
; unlike watching the counter it does not care how short the period is.
; The cost is the arming sequence, about 25 cycles, added to each bit --
; 1.5% at 600 baud, which drifts the last bit boundary 15% of a bit. The
; receiver samples at bit centres, so that is comfortably inside.
txarm:  lda btlo
        sta C2TALO
        lda bthi
        sta C2TAHI
        lda #25             ; force load, one-shot, start
        sta C2CRA
        rts
txwait: lda C2CRA
        and #1
        bne txwait          ; START still set: still counting
        rts
