...
; ===== boot lamp sweep =================================================
; The sweep was also a ~2.9 second delay before the port comes up, and
; deleting the lamps deleted the wait as a side effect -- which broke
; reception outright in the emulator. Keep the time, lose the lamps.
ledboot:
        lda #120            ; lamps never driven in this build
        sta C2DDRB
        lda #0
        sta C2PRB
        ldx #144            ; the same wait the sweep used to take
lbdly:  jsr wait1
        dex
        bne lbdly
        rts
