...
EQU = {
    ...
    "RXC":     0xCFFC,
    "GLC":     0xCFFA,   # DIAGNOSTIC: bytes received
...
splash: rts                 ; TUNER BUILD: no splash
        lda blankb
...
ploop:  ldy RTAIL
        ...
        lda RBUF,y
        inc RTAIL
        pha                 ; DIAGNOSTIC: the byte value itself. The
        jsr hexout          ; counters cannot tell noise from mis-sampling;
        lda #32             ; the values can.
        jsr CHROUT
        pla
...
; ===== TUNER =========================================================
; + / -   bit period, 8 cycles at a time
; . / ,   sample point, 16 cycles at a time
; =       print the current pair
; Both take effect on the very next byte, so turn a knob and watch the
; hex. When the values are right the bytes stop being rubbish and 3e
; starts appearing.
kbplus: lda btlo
        clc
        adc #8
...
; **EVERY LAMP WRITE IS DISABLED IN THIS BUILD.**
;
; The one build that connects on real hardware -- the kernal one -- is
; also the only one with the lamps off. Six LEDs switching current on the
; cartridge, on pins next to RXD and FLAG, while a 600-baud signal
; arrives, is a noise source that no emulator models. 212 edges rejected
; as glitches in one run is consistent with exactly that.
;
; Nothing here touches $dd01 now. If this connects, the lamps were the
; problem and they come back driven differently -- not at all during a
; byte, or on a port that is not the serial one.
ledrx:  rts
ledrxq: lda #0                  ; a ripple: the mesh is talking
        sta LEDPAT
        lda #0
        sta LEDIX
        lda #1
        ...
        rts
ledtx:  rts
ledtxq: lda #5                  ; all four at once: that one was ours
        sta LEDPAT
        lda #0
        sta LEDIX
        lda #1
...
; ===== boot lamp sweep =================================================
ledboot:
        lda #120            ; lamps never driven in this build
        sta C2DDRB
        lda #0
        sta C2PRB
        rts
...
status: lda #13
        ...
        lda #32
        jsr CHROUT
        lda #103            ; 'g' = edges rejected as glitches
        jsr CHROUT
        lda GLC
        jsr hexout
        jsr shoprm
...
def artwork():
    ...
    # TUNER BUILD: no artwork. The splash is 2.4K and this image has to
    # stay under $4000; the guard caught it at 15 bytes over.
    return ("\nlogocol: .byte 0\nlogodat: .byte 0\n")
