...
; -- two seconds settling after OPEN on top of this -- and it connects.
        ldx #150
rsyncw: jsr poll
        lda #0
        sta READY
...
; **Retry the first exchange.** There was no retry anywhere in the
; handshake: waitop simply returns when it times out, so a single lost
; APP_START -- because the radio was not listening yet, or the frame was
; corrupted -- left the client waiting forever with nothing to show. Four
; tries at eight seconds rather than one at thirty.
        lda #0
        sta HSRTY
        lda #2              ; short timeout while we are still retrying
        sta WTMO
hsrty:  lda #<pyastart
        sta PTR
        lda #>pyastart
        ...
        cmp #4
        bcc hsrty
hsgot:  lda #7              ; back to the patient timeout for the rest
        sta WTMO
