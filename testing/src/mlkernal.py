...
EQU = {
    ...
    "CHKOUT":  0xFFC9,
    "CLRCHN":  0xFFCC,
    "RIBUF":   0x00F7,   # -> kernal rs232 receive buffer
    "RIDBE":   0x029B,   # write index
    "RIDBS":   0x029C,   # read index
    "GETIN":   0xFFE4,
    "VICCTL":  0xD011,
    "JIFFY":   0x00A2,
    "NMIVEC":  0x0318,
    ...
    "MNN":     0xCFB3,
    "MNSEL":   0xCFB4,
    "MNPTR":   0x0024,   # -> table of string pointers; MUST be zero page
    "MNPTRH":  0x0025,
...
serini:
; KERNAL RS-232 instead of our own driver, so the driver can be ruled out
; as the cause of the hardware failure. Everything above this line -- the
; frame assembler, the ui, the archive -- is untouched.
        lda #2              ; logical file 2
        ldx #2              ; device 2 = rs232
        ldy #3              ; secondary 3, exactly as v1.2d opened it
        jsr SETLFS
        lda #2              ; two control bytes as the "filename"
        ldx #<rsctl
        ldy #>rsctl
        jsr SETNAM
        jsr KOPEN
; **OPEN does not arm the receiver. CHKIN does.** This cost two runs to
; find the first time round, and nothing in the manual says it.
        ldx #2
        jsr CHKIN
; **Then hand the input channel straight back.** CHKIN arms the receiver
; and CLRCHN does NOT disarm it -- but while file 2 is the input channel,
; GETIN reads the RADIO instead of the keyboard. Incoming mesh traffic was
; being typed into the input line, and every byte that happened to be 13
; sent the accumulated garbage out as a chat message. poll() reads the
; kernal's buffer directly through RIDBS/RIDBE, so nothing needs the
; channel to stay open.
        jsr CLRCHN
; **Then wait two seconds before trusting the link.** v1.2d records this
; as a kernal quirk: bytes sent or received in the moments after OPEN come
; out corrupted, because the nmi-driven engine is not fully armed yet. The
; first version of this build skipped the wait and put garbage on the wire
; -- the radio saw frames that decoded as random channel messages.
        ldx #120            ; ~2 seconds at 50Hz
srwait: jsr wait1
        dex
        bne srwait
        rts

; m51ctr: low nibble 7 = 600 baud, bits 5-6 clear = 8 data bits, bit 7
; clear = 1 stop bit. m51cdr 0 = 3-line handshake, no parity.
rsctl:  .byte 7, 0
...
sendfr:
; CHKOUT, then plain CHROUT per byte -- the kernal buffers it and its own
; NMI clocks it out. No timers, no masking the receiver: half duplex was
; our driver's constraint, not the kernal's.
        ldx #2
        jsr CHKOUT
        lda #60             ; '<'
        jsr CHROUT
...
sfl:    lda SFY
        ...
        inc SFY
        jmp sfl
sfd:    jsr CLRCHN          ; back to screen out, keyboard in
        rts
SFY:    .byte 0
...
poll:   lda READY
        ...
        sta OVR
pnohw:
ploop:  lda RIDBS           ; kernal ring: RIDBS reads, RIDBE writes
        cmp RIDBE
        beq pdone
        ldy RIDBS
        lda (RIBUF),y
...
ledstep:
; Lamps are OFF in this build, deliberately. Every lamp write puts a whole
; byte into $dd01, and under the kernal PB1 and PB2 are RTS and DTR --
; OUTPUTS. Writing zeros there drives them low and kills reception, which
; is the exact fault recorded twice on this project. The lamps are not
; what this build is testing, so the simplest safe thing is not to touch
; the port at all.
        rts
        lda LEDON
        beq lsidle
