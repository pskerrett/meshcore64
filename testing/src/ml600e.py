...
; **Drive RTS and DTR, as the kernal does.** 120 leaves PB1 and PB2 as
; floating inputs. The kernal's OPEN makes them driven outputs and
; asserts them -- and on the real cartridge PB1 is the pin next to PB0
; (RXD), which is next to FLAG, the line whose edges we detect. Two
; floating pins beside it are inert in an emulator and not necessarily
; inert in copper. 126 = PB1-PB6 out.
        lda #126
        sta C2DDRB
        lda C2PRB
        ora #6              ; RTS and DTR HIGH: asserted. Low tells the
        sta C2PRB           ; peer to stop sending and kills reception.
